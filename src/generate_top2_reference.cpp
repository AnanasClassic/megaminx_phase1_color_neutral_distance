...
std::vector<State> bfs(State root,int idx){
 ...
 std::vector<State>a;for(uint32_t h=0;h<N;h++)if(dist[h]>=dep-1)a.push_back(decode(h));
 std::printf("block %d root c%d e%d,%d,%d solved_eccentricity %u top2 %zu seen %llu\n",idx,root.cp,root.ep[0],root.ep[1],root.ep[2],dep,a.size(),(unsigned long long)seen);std::fflush(stdout);
 return a;
}
int main(int argc,char**argv){
 ...
 std::filesystem::path output=argc>3?argv[3]:"generated_reference/top2";
